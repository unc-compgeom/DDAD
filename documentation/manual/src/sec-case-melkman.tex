...
\texttt{IGeometryObserver} as the last input argument; in our case, the function
signature will be \texttt{Polygon\_2r Melkman(const Polyline\_2r\& P,
IGeometryObserver* observer)}. The only thing we need to do with the
\texttt{observer} is to have it observe the input polyline and output polygon
before we perform any operations on them.

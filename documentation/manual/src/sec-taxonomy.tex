...
% Dobkin outlines a number of design parameters relevant to workbench systems.
% Below, we synthesize Dobkin's specific design parameters with Price et. al.'s SV
% taxonomy~\cite{price1993principled}. Our new taxonomy comprises five categories:
% scope, content, form, method, and interaction.
...
Our taxonomy comprises five categories: scope, content, form, method, and
...
% This makes using the taxonomy to review existing systems and to explain the
% desired properties of the DDAD workbench system easier, as we simply answer each
% question in turn.

% This makes
% using the taxonomy easier to use, as we may simply answer each question in turn.

% Each category is defined in terms of a question about the system. By answering
% each question, we can easily use the taxonomy to..


% By defining each category in terms of a question about the system,
% we can more easily apply the taxonomy to review the properties of existing
% systems and to explain the desired properties of the DDAD workbench system.
...
% All
% geometric workbench systems specialize in visualizing geometric algorithms, but
% each system makes different choices with regards to scalability. Scalability
% refers to both input data and algorithm complexity.
...
% In particular, systems may focus on visualizing the actual
...
algorithm visualizer. \emph{Geo\-metric algorithm libraries} provide a broad
...
algorithms and using animations for teaching and demonstration.
...
\subsection{Degree-driven Algorithm Design and the DDAD Library}
...
implementation~\cite{liotta1998robust}. They called this design technique
\emph{degree-driven algorithm design}. Designers who consider the time, space,
and precision for a problem up-front arrive at new solutions, gain further
insight, and find simpler representations~\cite{millman2012degree}.

Momentarily, we explain the desiderata of the DDAD workbench in terms of the
taxonomy defined in section~\ref{sec:taxonomy}. First, we elaborate on the
concept of degree-driven algorithm design by examining the nature of geometric
algorithms and the geometric objects upon which they operate.

% and how the DDAD library supports implementing algorithms with
% low-degree predicates.
...
two line segments, or produce an offset of a polyhedral face.

In order for an algorithm implementation to perform correctly, the implementor
must ensure that all predicates evaluate correctly. Predicates are often phrased
as the sign of a polynomial expression over input coordinates. The degree of
these predicates may be defined as the degree of the corresponding polynomial.
In turn, the degree of an algorithm may be defined as the largest degree of any
predicate used. A degree $d$ predicate operating on $b$-bit integer input
coordinates requires roughly $db$ bits to evaluate correctly.
...
degree-driven geometric algorithms. In contrast to comprehensive libraries
...
implementations of common predicates, constructions, and geometric objects in
two dimensions.

% 
% 
% In most instances, the numerical data composing geometric
% objects is an approximation of a real value. Predicates that assume exact values
% but are fed approximate values are liable to make incorrect determinations.
% Constructions compound the situation by producing even coarser approximations.

% Predicates are often the sign of a polynomial and can be made correct by
% ensuring that the sign is evaluated with enough precision
...
% 
% 
%  In short, geometric nonrobustness is a problem wherein branching
...
% difficult. Geometric algorithms are often designed for Real-RAM, a computational
% model that provides arbitrary precision arithmetic operations at unit cost.
% Actual commodity hardware provides only finite precision and may result in
...
% space, an algorithm designer should also consider the arithmetic precision
% necessary to guarantee a correct implementation. They called this design
% technique degree-driven algorithm design. Designers who consider the time,
% space, and precision for a problem up-front arrive at new solutions, gain
% further insight, and find simpler representations.



\subsection{DDAD Workbench Desiderata}
...
\paragraph{Scope.} First, the system should focus on visualizing 2D algorithms,
with an eye toward visualizing 3D algorithms in the future. Second, the system
should provide example visualizations of textbook geometric algorithms. Third,
the system should be flexible enough to visualize new geometric algorithms from
the facilities used in example visualizations. Fourth, the system should run
well given inputs large enough to verify the correctness of a particular
implementation.

% First, the system should visualize geometric algorithms
...
pertinent for problems with dual formulations.

% First, the output medium should be an interactive computer
...
% programs can facilitate producing this type of output.
...
for 3D algorithms and larger data sets. Third, users should be able to visually
specify input data using a CAD-like interface and export it for re-use.

% Third, users should be able to record
